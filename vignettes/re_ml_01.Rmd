...
### 1. Code from [Chapter 10](https://geco-bern.github.io/agds_book/supervised_ml_I.html) from the AGDS I book

```{r libraries, message=FALSE, warning=FALSE}
# load libraries
library(dplyr);library(tidyverse);library(caret);library(recipes)
library(yardstick);library(gridExtra);library(scales)
```
...
### 1. Implications of k approaching 1 and the number of observations $N$
...
Another method to compute the optimal value for k is taking the **square root of the number of observations in the training data set and dividing them by two**, which is more a rule of thumb::
...
Cross-validation across multiple training and test sets return an optimal k, which is higher than what was returned from the initial training-test set (Figure 5). This indicates that there was a bias in our conclusion based on our choice of sample initially. In summary, a combination of the lowest MAE score with cross-validation yields the best estimate of the optimal value for k. Other methods like the elbow methods and Kneedle algorithm are available to provide efficient first guesses. 
