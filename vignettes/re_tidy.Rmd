...
First, I determined the log-response ratio for each observation of every study individually. The formula used for calculating the log-response ratio RR is the following with $x$ indicating the concentration of soil organic carbon in the two scenarios:
$$RR = ln( \frac{x_{elevated}}{x_{ambient}} )$$


Next, I aggregated the log-response ratios across all experiments with respect to the 3 previously defined phases using the following steps:
...
data_RR_means <- data_aggregated |>
  ...
  summarize(
    mean_RR_early = mean(RR_early[!is.nan(RR_early) & RR_early != 0], na.rm = T),         # early
