...
table_variables <- data.frame(
  Variable    = c("Name", "Class", "unit"), 
  GPP_NT_VUT_REF = c("Gross Primary Production", "numeric", "μmol CO2 /m\u00B2 s"),
  siteid      = c("Identification of site", "factor", "unitless"),
  TA_F        = c("Air temperature", "numeric", "°C"),
  TA_F_MDS    = c("Air temperature (gap filled)", "numeric", "°C"),
  SW_IN_F     = c("Incoming shortwave radiation", "numeric", "W/m\u00B2"),
  SW_IN_F_MDS = c("Incoming shortwave radiation (gap filled)", "numeric", "W/m\u00B2"),
  LW_IN_F     = c("Incoming longwave radiation", "numeric", "W/m\u00B2"),
  LW_IN_F_MDS = c("Incoming longwave radiation (gap filled)", "numeric", "W/m\u00B2"),
  VPD_F       = c("Vapor pressure deficit", "numeric", "hPa"),
  VPD_F_MDS   = c("Vapor pressure deficit (gap filled)", "numeric", "hPa"),
  PA_F        = c("Atmospheric pressure", "numeric", "kPa"),
  P_F         = c("Precipitation", "numeric", "mm"),
  WS_F        = c("Wind speed", "numeric", "m/s"),
  CO2_F_MDS   = c("CO2 mole fraction (gap filled)", "numeric", "unitless"),
  PPFD_IN     = c("Photosynthetic photon flux density", "numeric", "μmol Photon/m\u00B2 s"),
  USTAR       = c("Friction velocity", "numeric", "unitless"),
...
colnames(table_variables) <- c("Description", "Class", "Unit")

# Now, use kable to display the table with the proper column names
table_variables[2:nrow(table_variables), ] |>
  kable(caption = "Table 1: Variables", col.names = c("Variable", "Description", "Class")) |>
...
### 2. Methods
To investigate which single variables predicts GPP best, I fit linear models for each bivariate combination with GPP as the outcome and compare their $R^2$ and AIC scores.
...
3. Select the model with $p$ predictor(s) that achieves the highest $R^2$ (best fitting model) and compute its AIC.
...
Using the following function, I calculated the capacity of the linear models to predict GPP, using $R^2$, adjusted $R^2$, and AIC.
...
vars      <- colnames(half_hourly_fluxes |> select(-starts_with("GPP")))
...
After running this loop, I delete the first row, which has no predictors and the last row, which performed worse than the previous row.

```{r stepwise_cleaning, message=FALSE, warning=FALSE}
# clean up the dataframe
stepwise <- stepwise |>
  
  # remove first (empty) and last (not best) performing model
  slice(2:(n() - 1)) |> 
  
  # remove " + " at the start of predictors
  mutate(predictor = substr(predictor, 5, nchar(predictor)),
         index = seq_along(predictor)) |>
...
results_1 |>
  ...
  kable(caption = "Table 2: Model Performance Metrics", 
        digits = 3, col.names = c("Index", "Predictor", "R\u00B2","AIC"), align = rep("l", 4)) |>
  kable_styling(full_width = FALSE, position = "center")|>
  scroll_box(width = "100%")
```
...
plots_1 <- map2(vars_ordered, seq_along(vars_ordered), ~ {
  ...
  x_max <- if (is.numeric(x_vals)) max(x_vals, na.rm = TRUE) else tail(levels(x_vals), 1)
  y_min <- min(half_hourly_fluxes$GPP_NT_VUT_REF, na.rm = TRUE)
...
```
<center>
*Figure 2: Adjusted $R^2$ scores for the best performing model with a defined number of predictors from the stepwise regression.*
</center>
