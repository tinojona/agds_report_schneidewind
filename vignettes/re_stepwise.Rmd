...
```{r tablevariables, echo=FALSE, cache=TRUE}
...
```{r exerc1, warning=FALSE, message=FALSE, fig.height= 22, fig.width= 10, fig.align='center', cache=TRUE}
...
```{r stepwise, message=FALSE, warning=FALSE, cache=TRUE}
...
```{r results1, warning=FALSE, message=FALSE, echo=FALSE, cache=TRUE}
# present table
results_1 |>
  select(-R2adj) |>
  mutate(index = seq_along(predictor)) |>
...
```{r results1plots, warning=FALSE, message=FALSE, fig.height= 12, fig.width= 8, fig.align='center', echo=FALSE, cache=TRUE}
# display plots
